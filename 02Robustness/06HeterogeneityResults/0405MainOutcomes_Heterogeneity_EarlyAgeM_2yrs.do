...
esttab TenureMHigh_CSGC TenureMHigh_TSJVC TenureMHigh_PWLC TenureMHigh_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
...
esttab SameOffice_CSGC SameOffice_TSJVC SameOffice_PWLC SameOffice_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
...
esttab Young_CSGC Young_TSJVC Young_PWLC Young_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Worker age, young") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab TenureLow_CSGC TenureLow_TSJVC TenureLow_PWLC TenureLow_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Worker tenure, low") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab SameGender_CSGC SameGender_TSJVC SameGender_PWLC SameGender_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab OfficeSizeHigh_CSGC OfficeSizeHigh_TSJVC OfficeSizeHigh_PWLC OfficeSizeHigh_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab JobNum_CSGC JobNum_TSJVC JobNum_PWLC JobNum_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Office job diversity, high") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab LaborRegHigh_CSGC LaborRegHigh_TSJVC LaborRegHigh_PWLC LaborRegHigh_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Labor laws, high") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab LowFLFP_CSGC LowFLFP_TSJVC LowFLFP_PWLC LowFLFP_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab WPerf_CSGC WPerf_TSJVC WPerf_PWLC WPerf_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab WPerf0p10p90_CSGC WPerf0p10p90_TSJVC WPerf0p10p90_PWLC WPerf0p10p90_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Worker performance, high (p90)") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab TeamPerfMBase_CSGC TeamPerfMBase_TSJVC TeamPerfMBase_PWLC TeamPerfMBase_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Team performance, high (p50)") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab DiffM2y_CSGC DiffM2y_TSJVC DiffM2y_PWLC DiffM2y_Exit using "${Results}/MainOutcomes_Hetero_SelfConstructedData_2yrs.tex" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
