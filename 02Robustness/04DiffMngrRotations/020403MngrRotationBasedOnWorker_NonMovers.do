...
This do file computes manager-level average duration with workers inside a team for post-event workers.
    First, for each event worker, I first calculate his time with the post-event worker. 
    Then, for each post-event manager, I caculate the average exporsure time across event workers with him as the manager.
    I then plot the cdf for the post-event workers.

    This do file conducts extension (1).

Note:
    To avoid right-censoring, we focus only on workers with event time <= year 2019.

Two extensions:
    (1) only using those event workers who didn't change job after the event to calculate average exporsure time for each manager.
...
Output:
    "${TempData}/temp_EventWorkers_ExposureWithMngrs_NonMovers.dta"
    "${TempData}/temp_PostEventMngrID_EventWorkerID_NonMovers.dta"
    "${TempData}/temp_Mngr_Exposure_NonMovers.dta"
...
*!! focus only on non-movers (after the event)
replace TransferSJ              = 0 if FT_Rel_Time<=0 

sort IDlse YearMonth 
bysort IDlse: egen Transfer_Post = max(TransferSJ)
keep if Transfer_Post==0
    //&? keep only those workers who made no job transfer after the event 
...
save "${TempData}/temp_EventWorkers_ExposureWithMngrs_NonMovers.dta", replace 
...
save "${TempData}/temp_PostEventMngrID_EventWorkerID_NonMovers.dta", replace 

*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*?? step 3. aggregate worker-level exposure time to manager level 
*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??

use "${TempData}/temp_PostEventMngrID_EventWorkerID_NonMovers.dta", clear 
merge 1:1 IDlse using "${TempData}/temp_EventWorkers_ExposureWithMngrs_NonMovers.dta", nogenerate 
...
generate q_exposure_NonMovers = (inrange(FT_Mngr_Exposure, 15, 30))

rename IDlseMHR Post_Mngr_ID

save "${TempData}/temp_Mngr_Exposure_NonMovers.dta", replace 
...
graph export "${Results}/DistOfMngrExposureWithWorkers_NonMovers.png", replace as(png)
