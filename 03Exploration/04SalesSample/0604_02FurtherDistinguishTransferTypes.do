...
sort IDlse YearMonth
bysort IDlse: egen Movers                 = max(cond(inrange(FT_Rel_Time, 0, 24),  AE_TransferSJ, .))
bysort IDlse: egen WithinTeamMovers       = max(cond(inrange(FT_Rel_Time, 0, 24),  AE_TransferSJSameMSameFunc, .))
bysort IDlse: egen DiffFuncMovers         = max(cond(inrange(FT_Rel_Time, 0, 24),  AE_TransferFunc, .))
bysort IDlse: egen DiffTeamSameFuncMovers = max(cond(inrange(FT_Rel_Time, 0, 24),  AE_TransferSJDiffMSameFunc, .))
...
esttab Prod Prod_Movers LogPayBonus_Movers LogPayBonus_WTM LogPayBonus_DTM LogPayBonus_DFM using "${Results}/ResultsConditionalOnDiffTypesOfLateralMoves.tex", ///
