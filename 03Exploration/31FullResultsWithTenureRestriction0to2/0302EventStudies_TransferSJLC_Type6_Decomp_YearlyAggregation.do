...
Special notes:
    ...
    (2) Only those employees whose tenure at the event time is in the range of [0,2] are included.
...
graph bar coef_SameMLC_gains coef_DiffMLC_gains coef_DiffFuncSJLC_gains, ///
    ...
    ylabel(-0.03(0.01)0.15, grid gstyle(dot)) ///
    text(0.150 1.00 "reporting % of total lateral moves", size(medium) placement(e)) ///
    text(0.007 9.50 "${frac_DiffMLC_1}"     , size(vsmall)  placement(c)) ///
    text(0.012 9.50 "${frac_DiffFuncSJLC_1}", size(vsmall)  placement(c)) ///
    text(0.02  23.0 "${frac_DiffMLC_2}"     , size(medium)  placement(c)) ///
    text(0.04  23.0 "${frac_DiffFuncSJLC_2}", size(medium)  placement(c)) ///
    text(0.05  36.5 "${frac_DiffMLC_3}"     , size(medium)  placement(c)) ///
    text(0.075 36.5 "${frac_DiffFuncSJLC_3}", size(medium)  placement(c)) ///
    text(0.04  50.5 "${frac_DiffMLC_4}"     , size(medium)  placement(c)) ///
    text(0.075 50.5 "${frac_DiffFuncSJLC_4}", size(medium)  placement(c)) ///
    text(0.05  64.0 "${frac_DiffMLC_5}"     , size(medium)  placement(c)) /// // text(0.10  64.0 "${frac_DiffFuncSJLC_5}", size(medium)  placement(c)) ///
    ...
    text(0.11  91.0 "${frac_DiffFuncSJLC_7}", size(medium)  placement(c))
graph export "${EventStudyResults}/CA30_Outcome2_TransferSJLC_Decomp_YearlyAggregation.pdf", replace as(pdf)
