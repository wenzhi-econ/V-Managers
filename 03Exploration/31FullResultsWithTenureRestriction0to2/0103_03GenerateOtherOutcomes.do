...
label variable TenureAtEvent "Employee's tenure (in years) at the event time"
...
keep ///
    ...
    TenureAtEvent ///
    TransferSJ TransferSJC TransferSJL TransferSJLC ///
...
drop ///
    ONETSOCCode ///
    intensity_cognitive_a intensity_cognitive_b intensity_cognitive_c intensity_cognitive_d intensity_cognitive_e intensity_cognitive_f ///
    intensity_routine_g intensity_routine_h ///
    intensity_social_i intensity_social_j intensity_social_k intensity_social_l ///
    intensity_cognitive intensity_routine intensity_social ///
    prank_cognitive prank_routine prank_social
...
generate Post_TransferSJL = TransferSJL
replace  Post_TransferSJL = 0 if Rel_Time<0

*!! a cumulative count of post-event lateral transfers 
generate temp = Post_TransferSJL
by IDlse (YearMonth), sort: replace temp = temp[_n] + temp[_n-1] if _n>1 
replace temp = 0 if temp==. & StandardJob!=""
generate Post_TransferSJLC = temp 
drop temp

*!! making at least one post-event lateral transfer 
generate SJLAtLeast1 = .
replace  SJLAtLeast1 = 0 if Rel_Time>=0 & Post_TransferSJLC==0
replace  SJLAtLeast1 = 1 if Rel_Time>=0 & Post_TransferSJLC>=1

*!! drop and order 
drop Post_TransferSJL Post_TransferSJLC
order SJLAtLeast1, after(TransferSJLC)
label variable SJLAtLeast1 "=1 from the month onwards when a first post-event TransferSJL occurs"
...
label variable TransferSJC               "Cumulative count of all job changes"
label variable TransferSJSameMSameFuncC  "Cumulative count of within team job changes"
label variable TransferSJDiffMSameFuncC  "Cumulative count of diff. team, same function job changes"
label variable TransferFuncC             "Cumulative count of diff. team, different function job changes"
label variable ChangeM                   "= 1 in months when an individual's manager is diff. than last months"

*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*?? step 4. decompose TransferSJL into three categories:
*??         (1) within team (same manager, same function)
*??         (2) different team (different manager), and different function
*??         (3) different team (different manager), but same function
*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
...
/* 
Notes: 
    (1) This decomposition is based on step 3 decomposition. 
    (2) The only change is to adjust to simultaneous work level promotions.
*/

sort IDlse YearMonth
foreach var in TransferSJSameMSameFunc TransferSJDiffMSameFunc TransferFunc {

    if "`var'" == "TransferSJSameMSameFunc" local newvar SameML
    if "`var'" == "TransferSJDiffMSameFunc" local newvar DiffML
...
order SameMLC DiffMLC DiffFuncSJLC, after(TransferSJLC)

*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*?? step 5. save the final dataset for event studies 
*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
