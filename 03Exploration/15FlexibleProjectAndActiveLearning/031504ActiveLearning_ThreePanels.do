...
esttab NumSkillsB0 NumCompletedYTDB0 NumRecommendYTDB0 ActiveLearnerYTD0 using "${Results}/FTActiveLearn_ThreePanel_035.tex", ///
...
esttab NumSkillsB3 NumCompletedYTDB3 NumRecommendYTDB3 ActiveLearnerYTD3 using "${Results}/FTActiveLearn_ThreePanel_035.tex", ///
...
esttab NumSkillsB5 NumCompletedYTDB5 NumRecommendYTDB5 ActiveLearnerYTD5 using "${Results}/FTActiveLearn_ThreePanel_035.tex", ///
    append style(tex) fragment nocons label nofloat nobaselevels se nonumbers ///
    nomtitles collabels(,none) ///
    stats(cmean N, labels("Mean, low-flyer" "N") fmt(%9.3f %9.0f)) ///
    prehead("") ///
    posthead("" ) ///
    postfoot("\hline" "\end{tabular}" "\begin{tablenotes}" "\footnotesize" "\item" "Notes. An observation is a worker-year-month. Standard errors are clustered by manager. Data from the internal talent matching platform. \emph{Number of skills $\geq$ 3} equals to 1 if the worker has more than 3 skills in the platform. \emph{Completed items $\geq$ 5} equals to 1 if the worker has completed more than 5 items in the platform. \emph{Shared items with colleagues $>$ 0} equals to 1 if the worker has done items with colleagues. \emph{Active learner} equals to 1 if the worker meets all the above three conditions. In panel (a), the regressor is the employee's current manager type; while in panels (b) and (c), the regressor is the employee's lagged manager type. Controls include country and year FE." "\end{tablenotes}")