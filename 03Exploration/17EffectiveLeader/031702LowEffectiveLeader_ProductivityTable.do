...
-------------------------------------------------------------
 ...
 1%            2              1
 5%     2.905263              1
10%          3.2              1       Obs             178,344
25%         3.85              1       Sum of wgt.     178,344

50%     4.230769                      Mean           4.125139
                        Largest       Std. dev.      .6963943
75%     4.610169              5
90%            5              5       Variance        .484965
95%            5              5       Skewness      -1.330644
99%            5              5       Kurtosis        5.62872
*/
generate MScoreB = (MScore>4) if MScore!=.
generate MScoreLow = (MScore==1) if MScore!=.
...
reghdfe MScoreLow FT_LtoH ${control_vars} if WL==2 & FT_Mngr_both_WL2==1 & (FT_LtoL==1 | FT_LtoH==1) & FT_Post==1, cluster(IDlseMHR) a(${absorb_vars})
    eststo MScoreLow_Gain
    summarize MScoreLow if e(sample)==1 & FT_LtoL==1
    estadd scalar cmean = r(mean)
...
esttab Prod Prod_Movers LogPayBonus_Movers LogPayBonus_Gain MScore_Gain MScoreB_Gain MScoreLow_Gain using "${Results}/ResultsConditionalOnLateralAndVerticalMoves_LowEffectiveLeader.tex", ///
