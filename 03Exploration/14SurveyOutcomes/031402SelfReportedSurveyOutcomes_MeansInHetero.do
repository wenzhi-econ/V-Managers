...
esttab LineManagerB UteampcMean UfocuspcMean UhappypcMean UcompanypcMean using "${Results}/FiveSurveyOutcomes_MeanAndMeanInHetero.tex", ///
    replace style(tex) fragment nocons label nofloat nobaselevels noobs ///
    nomtitles collabels(,none) ///
    star(* 0.10 ** 0.05 *** 0.01) ///
    keep(EarlyAgeM) ///
...
esttab LineManagerB_Heter UteampcMean_Heter UfocuspcMean_Heter UhappypcMean_Heter UcompanypcMean_Heter using "${Results}/FiveSurveyOutcomes_MeanAndMeanInHetero.tex", ///
    ...
    prefoot("\hline") ///
    postfoot("\hline" "\hline" "\end{tabular}" "\begin{tablenotes}" "\footnotesize" "\item" "Notes. An observation is a worker-year. Data from the annual pulse survey run by the firm since 2017. Standard errors are clustered by manager. Controls include: the interaction between age band and gender FE, and country and year FE. In Panel (a) and (b), survey indices are the average of various survey questions, grouped together by theme as detailed in Table S.3. I use binary variables to construct the averages: probability of answering 5 out of 5-point Likert Scale. In Panel (a), estimates obtained by running the model in equation S.1. In Panel (b), estimates obtained by running the model in equation S.1 interacting indicator for high-flyer manager with an indicator for whether the worker changes job." "\end{tablenotes}")
