...
esttab LineManagerB Uteampc1 Ufocuspc1 Uhappypc1 Ucompanypc1 using "${Results}/FiveSurveyOutcomes_PCA_SameControlsAsExits.tex", ///
    ...
    posthead("\hline") ///
    prefoot("\hline") ///
    postfoot("\hline" "\hline" "\end{tabular}" "\begin{tablenotes}" "\footnotesize" "\item" "Notes. An observation is a worker-year. Data from the annual pulse survey run by the firm since 2017. Standard errors are clustered by manager.Control variables include the fixed effects of the interaction of office and function, as well as the interaction between age band and gender, and year fixed effects. Survey indices are the first principal components of various survey questions, grouped together by theme as detailed in Table S.3. I use binary variables to construct the first principal components: probability of answering 5 out of 5-point Likert Scale. Estimates obtained by running the model in equation S.1." "\end{tablenotes}")

esttab LineManagerB UteampcMean UfocuspcMean UhappypcMean UcompanypcMean using "${Results}/FiveSurveyOutcomes_MeanAndHetero_SameControlsAsExits.tex", ///
    replace style(tex) fragment nocons label nofloat nobaselevels noobs ///
    nomtitles collabels(,none) ///
    star(* 0.10 ** 0.05 *** 0.01) ///
    keep(EarlyAgeM) ///
...
esttab LineManagerB_Heter Uteampc1_Heter Ufocuspc1_Heter Uhappypc1_Heter Ucompanypc1_Heter using "${Results}/FiveSurveyOutcomes_MeanAndHetero_SameControlsAsExits.tex", ///
    ...
    prefoot("\hline") ///
    postfoot("\hline" "\hline" "\end{tabular}" "\begin{tablenotes}" "\footnotesize" "\item" "Notes. An observation is a worker-year. Data from the annual pulse survey run by the firm since 2017. Standard errors are clustered by manager.Control variables include the fixed effects of the interaction of office and function, as well as the interaction between age band and gender, and year fixed effects. In Panel (a), survey indices are the average of various survey questions, grouped together by theme as detailed in Table S.3. I use binary variables to construct the averages: probability of answering 5 out of 5-point Likert Scale. Estimates obtained by running the model in equation S.1. In Panel (b), survey indices are the first principal components of various survey questions, grouped together by theme as detailed in Table S.3 and sample is restricted to the first year since the manager transition. Estimates obtained by running the model in equation S.1 interacting indicator for high-flyer manager with an indicator for whether the worker changes job." "\end{tablenotes}")
