/* 
This do file runs event studies on the task distance measures separately for cognitive, routine, and social tasks.
...
foreach var in cumdist_cognitive cumdist_routine cumdist_social {
    capture drop temp
...
foreach var in cumdist_cognitive cumdist_routine cumdist_social {
    ...
    if "`var'" == "cumdist_cognitive" global title "Cumulative distance in cognitive task intensity"
    if "`var'" == "cumdist_routine"   global title "Cumulative distance in routine task intensity"
    if "`var'" == "cumdist_social"    global title "Cumulative distance in social task intensity"
...
save "${Results}/FT_TaskDistanceMeasure_Separate.dta", replace 
