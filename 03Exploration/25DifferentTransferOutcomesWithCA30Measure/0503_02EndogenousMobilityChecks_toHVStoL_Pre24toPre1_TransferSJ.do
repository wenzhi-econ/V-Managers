...
global latex_file         "${Results}/004ResultsBasedOnCA30/CA30_EndogenousMobilityChecks_FullTransition_Pre24toPre1_TransferSJ.tex"
...
esttab reg1 reg2 reg3 reg4 using "${latex_file}", ///
    ...
    prehead("\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}" "\begin{tabular}{lcccc}" "\toprule" "\toprule" "\multicolumn{5}{c}{\textit{Panel (a): team performance}} \\ [7pt]" "& \multicolumn{1}{c}{(1)} & \multicolumn{1}{c}{(2)}  & \multicolumn{1}{c}{(3)} & \multicolumn{1}{c}{(4)} \\" "& \multicolumn{1}{c}{Salary (logs)} & \multicolumn{1}{c}{Salary grade increase}  & \multicolumn{1}{c}{Standard job change} & \multicolumn{1}{c}{Cross-functional move} \\") ///
    posthead("\midrule") ///
    prefoot("")  ///
    postfoot("\midrule")
