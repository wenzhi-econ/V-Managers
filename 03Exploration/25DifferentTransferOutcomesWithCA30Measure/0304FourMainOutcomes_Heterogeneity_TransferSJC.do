...
For ChangeSalaryGradeC TransferSJC outcomes, a simplified event study is implemented (months -1, -2, -3 are taken as the reference group).
...
Results:
    "${Results}/004ResultsBasedOnCA30/CA30_HeterogeneityInFourMainOutcomes_TransferSJC.tex"
...
Notes:
    ...
    (4) This won't affect regressions for other outcome variables, since we never include month 0 in "ChangeSalaryGradeC" and "TransferSJC" regressions.
*/
...
foreach hetero_var in $Hetero_Vars {
    ...
    if ("`hetero_var'" == "LowFLFP") {
        ...
        foreach outcome in ChangeSalaryGradeC TransferSJC PromWLC {
            ...
            xlincom (LtoHXPost1X`hetero_var' - LtoLXPost1X`hetero_var'), level(95) post
                ...
                if "`outcome'" == "TransferSJC" local outcome_name TSJVC
...
global latex_file "${Results}/004ResultsBasedOnCA30/CA30_HeterogeneityInFourMainOutcomes_TransferSJC.tex"

esttab TenureMHigh_CSGC TenureMHigh_TSJVC TenureMHigh_PWLC TenureMHigh_Exit using "${latex_file}" ///
    ...
    prehead("\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}" "\begin{tabular}{lcccc}" "\hline\hline" "& \multicolumn{1}{c}{Pay increase} & \multicolumn{1}{c}{Standard job changes} & \multicolumn{1}{c}{Vertical moves} & \multicolumn{1}{c}{Exit from firm} \\") ///
    posthead("\hline \multicolumn{3}{c}{\textit{Panel (a): worker and manager characteristics}} \\ \hline")
esttab SameOffice_CSGC SameOffice_TSJVC SameOffice_PWLC SameOffice_Exit using "${latex_file}" ///
...
esttab SameGender_CSGC SameGender_TSJVC SameGender_PWLC SameGender_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Same gender as manager") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab Young_CSGC Young_TSJVC Young_PWLC Young_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab OfficeSizeHigh_CSGC OfficeSizeHigh_TSJVC OfficeSizeHigh_PWLC OfficeSizeHigh_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab JobNum_CSGC JobNum_TSJVC JobNum_PWLC JobNum_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Office job diversity, high") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab LaborRegHigh_CSGC LaborRegHigh_TSJVC LaborRegHigh_PWLC LaborRegHigh_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Labor laws, high") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab LowFLFP_CSGC LowFLFP_TSJVC LowFLFP_PWLC LowFLFP_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab WPerf_CSGC WPerf_TSJVC WPerf_PWLC WPerf_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab WPerf0p10p90_CSGC WPerf0p10p90_TSJVC WPerf0p10p90_PWLC WPerf0p10p90_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
    keep(lc_1) coeflabels(lc_1 "Worker performance, high (p90)") ///
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("") posthead("") prefoot("") postfoot("")
esttab TeamPerfMBase_CSGC TeamPerfMBase_TSJVC TeamPerfMBase_PWLC TeamPerfMBase_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
