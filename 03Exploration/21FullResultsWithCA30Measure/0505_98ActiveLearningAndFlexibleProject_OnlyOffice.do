...
reghdfe NumRecommend CA30_LtoH, absorb(Year Office) vce(cluster IDMngr_Post)
    eststo NumRecommend
    summarize NumRecommend if e(sample)==1 & CA30_LtoH==0
    estadd scalar cmean = r(mean)

reghdfe NumCompleted CA30_LtoH, absorb(Year Office) vce(cluster IDMngr_Post)
    eststo NumCompleted
    summarize NumCompleted if e(sample)==1 & CA30_LtoH==0
    estadd scalar cmean = r(mean)

reghdfe NumSkills    CA30_LtoH, absorb(Year Office) vce(cluster IDMngr_Post)
    eststo NumSkills
    summarize NumSkills if e(sample)==1 & CA30_LtoH==0
    estadd scalar cmean = r(mean)

reghdfe ActiveLearner CA30_LtoH, absorb(Year Office) vce(cluster IDMngr_Post)
    eststo ActiveLearner
    summarize ActiveLearner if e(sample)==1 & CA30_LtoH==0
    estadd scalar cmean = r(mean)
...
foreach var in  CompletedProfileDummy ProjectRolesAppliedDummy NrProjectRolesApplied {
    reghdfe `var' CA30_LtoH, absorb(Event_Time Office) vce(cluster IDMngr_Post)
        eststo `var'
...
global latex_numbers_B    "& \multicolumn{1}{c}{(1)} & \multicolumn{1}{c}{(2)} & \multicolumn{1}{c}{(3)} & \multicolumn{1}{c}{(4)} \\"
...
global latex_titles_B     "& \multicolumn{1}{c}{\shortstack{Number of \\ skills}} & \multicolumn{1}{c}{\shortstack{Number of \\ completed items}}  & \multicolumn{1}{c}{\shortstack{Number of \\ shared items}} & \multicolumn{1}{c}{Active learner}  \\"
global latex_panel_A      "\addlinespace[5pt] \multicolumn{4}{c}{\emph{Panel (a): Active learning behavior}} \\ [7pt]"
global latex_panel_B      "\addlinespace[5pt] \multicolumn{4}{c}{\emph{Panel (b): Engagement in flexible projects}} \\ [7pt]"
global latex_file         "${Results}/004ResultsBasedOnCA30/ActiveLearningAndFlexibleProject_OnlyOffice.tex"
...
esttab NumSkills NumCompleted NumRecommend ActiveLearner using "${latex_file}", ///
