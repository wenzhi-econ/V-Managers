...
"""
First, separately for LtoL and LtoH workers, 2 and 7 years after the event, four occupation
transition ratio matrices are calculated.
Second, the difference between the LtoH and LtoL matrices are calculated.
Third, heatmaps are plotted for the two matrices storing the differences.
...
transition_dta = pd.read_stata(setup.data("070501_OccTransferMap.dta"), convert_categoricals=False)
...
tran_2yr_LtoL = calculate_transition_matrix(
    transition_dta.loc[(transition_dta["CA30_LtoL"] == 1)], 2
)
tran_2yr_LtoH = calculate_transition_matrix(
    transition_dta.loc[(transition_dta["CA30_LtoH"] == 1)], 2
)
tran_7yr_LtoL = calculate_transition_matrix(
    transition_dta.loc[(transition_dta["CA30_LtoL"] == 1)], 7
)
tran_7yr_LtoH = calculate_transition_matrix(
    transition_dta.loc[(transition_dta["CA30_LtoH"] == 1)], 7
)
...
# ??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??
# ?? step 3. draw the heat map for 2 and 7 years after the event
# ??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??#??
