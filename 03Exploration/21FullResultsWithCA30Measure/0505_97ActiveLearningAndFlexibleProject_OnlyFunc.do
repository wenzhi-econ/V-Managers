...
reghdfe NumRecommend CA30_LtoH, absorb(Year Func) vce(cluster IDMngr_Post)
    eststo NumRecommend
    summarize NumRecommend if e(sample)==1 & CA30_LtoH==0
    estadd scalar cmean = r(mean)

reghdfe NumCompleted CA30_LtoH, absorb(Year Func) vce(cluster IDMngr_Post)
    eststo NumCompleted
    summarize NumCompleted if e(sample)==1 & CA30_LtoH==0
    estadd scalar cmean = r(mean)

reghdfe NumSkills    CA30_LtoH, absorb(Year Func) vce(cluster IDMngr_Post)
    eststo NumSkills
    summarize NumSkills if e(sample)==1 & CA30_LtoH==0
    estadd scalar cmean = r(mean)

reghdfe ActiveLearner CA30_LtoH, absorb(Year Func) vce(cluster IDMngr_Post)
...
foreach var in  CompletedProfileDummy ProjectRolesAppliedDummy NrProjectRolesApplied {
    reghdfe `var' CA30_LtoH, absorb(Event_Time Func) vce(cluster IDMngr_Post)
        eststo `var'
...
global latex_file         "${Results}/004ResultsBasedOnCA30/ActiveLearningAndFlexibleProject_OnlyFunc.tex"
