...
codebook IDteam if spanM>1 & inrange(Rel_Time, -36, -6) // 3,472 teams
...
foreach y in  $perf $div $homo {
    ...
    reghdfe `y' CA30_LtoH CA30_HtoH CA30_HtoL if spanM>1 & inrange(Rel_Time, -36, -6), cluster(IDlseMHRPreMost) absorb($controls)
...
esttab reg1 reg2 reg3 reg4 using "${Results}/004ResultsBasedOnCA30/CA30_EndogenousMobilityChecks_FullTransition.tex", ///
...
esttab reg5 reg6 reg7 reg8 using "${Results}/004ResultsBasedOnCA30/CA30_EndogenousMobilityChecks_FullTransition.tex", ///
...
esttab reg9 reg10 reg11 reg12 using "${Results}/004ResultsBasedOnCA30/CA30_EndogenousMobilityChecks_FullTransition.tex", ///
    append style(tex) fragment nocons label nofloat nobaselevels se ///
    nomtitles collabels(,none) ///
    star(* 0.10 ** 0.05 *** 0.01) b(4) se(3) ///
    keep(lc_1 lc_2) order(lc_1 lc_2) varlabels(lc_1 "LtoH - LtoL" lc_2 "HtoL - HtoH") ///
