...
reghdfe Prod ${four_events_dummies} if ISOCode=="IND", absorb(IDlse YearMonth) vce(cluster IDlseMHR) 
    ...
    local cmean = r(mean)
    xlincom (coef_0y_to_2yr = ${coef_0y_to_2yr}) (coef_2yr_to_5yr = ${coef_2yr_to_5yr}), post 
        eststo Indian_Prod 
...
global latex_file         "${Results}/005EventStudiesWithCA30/CA30_ProdAndPayInEventStudies_60PostPeriods.tex"
...
esttab Indian_Prod Indian_LogPayBonus Full_ProductivityStd Full_LogPayBonus using "${latex_file}", ///
    ...
    keep(coef_0y_to_2yr coef_2yr_to_5yr) order(coef_0y_to_2yr coef_2yr_to_5yr) ///
    varlabels(coef_0y_to_2yr "Average effects 0-2 years after the event" coef_2yr_to_5yr "Average effects 2-5 years after the event") ///
