/* 
This do file runs event studies on the productivity outcomes on a subset of countries.
...
graph twoway ///
    (histogram Rel_Time, width(1) fraction discrete bcolor(ebblue%10)) ///
    , xlabel(-130(10)150, grid gstyle(dot) labsize(small)) title("Full event study sample")
graph export "${Results}/005EventStudiesWithCA30/DistributionOfRelTime_FullEventStudySample.pdf", replace as(pdf)
...
------------+-----------------------------------
        IND |     34,175       73.57       73.57
        IDN |      4,646       10.00       83.58
        ITA |      3,101        6.68       90.25
        RUS |      2,642        5.69       95.94
        MEX |        861        1.85       97.79
        PHL |        432        0.93       98.72
        GTM |        161        0.35       99.07
        MYS |         96        0.21       99.28
...
graph export "${Results}/005EventStudiesWithCA30/DistributionOfRelTimeAcrossCountriesInProdSample_Layout.pdf", replace as(pdf)
...
graph export "${Results}/005EventStudiesWithCA30/DistributionOfRelTimeAcrossCountriesInProdSample_Panel.pdf", replace as(pdf)
...
foreach var in ProductivityStd {
    ...
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
    ...
    reghdfe `var' ${four_events_dummies} if ISOCode=="RUS", absorb(IDlse YearMonth) vce(cluster IDlseMHR) 
    ...
    *!! quarterly estimates
    ...
    graph export "${Results}/005EventStudiesWithCA30/CA30_Outcome${number}_`var'_Coef1_Gains_Country4_OnlyRUS.pdf", replace as(pdf)
    ...
    *!! quarterly estimates
    ...
    graph export "${Results}/005EventStudiesWithCA30/CA30_Outcome${number}_`var'_Coef2_Loss_Country4_OnlyRUS.pdf", replace as(pdf)   
    ...
    *!! quarterly estimates
    ...
    graph export "${Results}/005EventStudiesWithCA30/CA30_Outcome${number}_`var'_Coef3_GainsMinusLoss_Country4_OnlyRUS.pdf", replace as(pdf)
}