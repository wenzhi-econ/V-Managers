...
keep IDlse YearMonth Rel_Time Event_Time CA30_LtoL CA30_LtoH Func StandardJob ONETOccTitle
...
/* 
    ...
    -----------------------------------------
    Not matched                       129,733
        from master                   128,955  (_merge==1)
        from using                        778  (_merge==2)

    Matched                         1,409,488  (_merge==3)
    -----------------------------------------
*/
...
tab ONETOccTitle if OccTask==1
codebook ONETOccTitle if OccTask==1
codebook StandardJob if OccTask==1

tab ONETOccTitle if OccTask==2
codebook ONETOccTitle if OccTask==2
codebook StandardJob if OccTask==2

tab ONETOccTitle if OccTask==3
codebook ONETOccTitle if OccTask==3
codebook StandardJob if OccTask==3
...
tab OccTask0
/* 
   OccTask0 |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |      6,568       31.14       31.14
          2 |      5,544       26.28       57.42
          3 |      8,983       42.58      100.00
------------+-----------------------------------
      Total |     21,095      100.00
*/

save "${TempData}/070501_OccTransferMap.dta", replace