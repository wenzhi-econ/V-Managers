...
Output:
    "${Results}/20250414log_EventStudiesWithCA30_Outcome6_DecompTransferSJC.txt"

Results:
    "${Results}/CA30_Gains_DecompTransferSJC_DuringMngrRotation_Q8.gph"
    "${Results}/CA30_Gains_DecompTransferSJC_AfterMngrRotation_Q28.gph"
...
capture log close
log using "${Results}/20250414log_EventStudiesWithCA30_Outcome6_DecompTransferSJC", replace text
...
foreach var in TransferSJC SameMC DiffMC TransferFuncC {
    ...
    if "`var'" == "TransferSJC" global number "9"

    if "`var'" == "SameMC" global title "Standard job change: Within team "
    if "`var'" == "SameMC" global number "10"

    if "`var'" == "DiffMC" global title "Standard job change: Different team, same function"
    if "`var'" == "DiffMC" global number "11"

    if "`var'" == "TransferFuncC" global title "Standard job change: Different team, different function"
    if "`var'" == "TransferFuncC" global number "12"
...
graph save "${Results}/CA30_Gains_DecompTransferSJC_DuringMngrRotation_Q8.gph", replace
