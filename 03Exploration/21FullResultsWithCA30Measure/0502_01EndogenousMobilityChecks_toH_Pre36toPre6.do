...
codebook IDteam if spanM>1 & inrange(Rel_Time, -36, -6) 
    //&? 3,427 teams

codebook IDteam if inrange(Rel_Time, -36, -6) 
    //&? 12,088 teams
...
foreach y in  $perf $div $homo {
    ...
    reghdfe `y' CA30_toH if spanM>1 & inrange(Rel_Time, -36, -6), cluster(IDlseMHRPreMost) absorb($controls)
        eststo reg`i'
...
esttab reg1 reg2 reg3 reg4 using "${Results}/004ResultsBasedOnCA30/CA30_EndogenousMobilityChecks_ToH.tex", ///
...
esttab reg5 reg6 reg7 reg8 using "${Results}/004ResultsBasedOnCA30/CA30_EndogenousMobilityChecks_ToH.tex", ///
...
esttab reg9 reg10 reg11 reg12 using "${Results}/004ResultsBasedOnCA30/CA30_EndogenousMobilityChecks_ToH.tex", ///
    append style(tex) fragment nocons label nofloat nobaselevels nonumbers noobs nomtitles collabels(,none) ///
    b(4) se(3) star(* 0.10 ** 0.05 *** 0.01) ///
    keep(CA30_toH) ///
    stats(mean_toL r2 N, labels("Mean, low-flyer manager" "R-squared" "N") fmt(%9.3f %9.3f %9.0g)) ///
