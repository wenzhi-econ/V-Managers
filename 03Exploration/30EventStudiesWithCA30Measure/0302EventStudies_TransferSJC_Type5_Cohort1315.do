/* 
This do file runs event study regressions on outcome: TransferSJC.
This do file establishes robustness in event study results, using a single cohort of event workers.
...
Output:
    "${EventStudyResults}/20250519log_Outcome2_TransferSJC_Type5_Cohort1315.txt"
    "${EventStudyResults}/Outcome2_TransferSJC_Type5_Cohort1315.dta"
...
capture log close
log using "${EventStudyResults}/20250519log_Outcome2_TransferSJC_Type5_Cohort1315.txt", replace text
...
generate cohortSingle = 1 if Event_Time<=tm(2015m12) & Event_Time>=tm(2013m1)

*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*?? step 2. event studies on the two main outcomes
*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??

foreach var in TransferSJC {

    if "`var'" == "TransferSJC" global title "Standard job change"
    if "`var'" == "TransferSJC" global number "2"
    ...
    *!! quarterly estimates
    ...
    graph save "${EventStudyResults}/CA30_Outcome${number}_`var'_Coef1_Gains_Type5_Cohort1315.gph", replace
    ...
    *!! quarterly estimates
    ...
    graph save "${EventStudyResults}/CA30_Outcome${number}_`var'_Coef2_Loss_Type5_Cohort1315.gph", replace   
    ...
    *!! quarterly estimates
    ...
    graph save "${EventStudyResults}/CA30_Outcome${number}_`var'_Coef3_GainsMinusLoss_Type5_Cohort1315.gph", replace
...
save "${EventStudyResults}/Outcome2_TransferSJC_Type5_Cohort1315.dta", replace 
