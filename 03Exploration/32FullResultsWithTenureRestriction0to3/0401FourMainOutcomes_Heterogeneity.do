...
For ChangeSalaryGradeC TransferSJLC outcomes, a simplified event study is implemented (months -1, -2, -3 are taken as the reference group).
...
Input:
    "${TempData}/0104AnalysisSample_TenureRestriction0to2_WithHeteroIndicators.dta" <== created in 0104 do file 

Results:
    "${Results}/004ResultsBasedOnCA30/CA30_HeterogeneityInFourMainOutcomes_TransferSJLC.tex"
...
use "${TempData}/0104AnalysisSample_TenureRestriction0to3_WithHeteroIndicators.dta", clear 
...
/* 
Notes:
    (1) The definition of "Post" in the above procedures is a different from the convention: Rel_Time==0 is not included in Post1. 
    ...
    (4) This won't affect regressions for other outcome variables, since we never include month 0 in "ChangeSalaryGradeC" and "TransferSJLC" regressions.
*/
...
foreach hetero_var in $Hetero_Vars {
    ...
    if ("`hetero_var'" == "LowFLFP") {
        ...
        foreach outcome in ChangeSalaryGradeC TransferSJLC PromWLC {
            ...
            xlincom (LtoHXPost1X`hetero_var' - LtoLXPost1X`hetero_var'), level(95) post
                ...
                if "`outcome'" == "TransferSJLC" local outcome_name TSJVC
...
global latex_file "${Results}/022FullResultsWithTenureRestriction0to3/CA30_HeterogeneityInFourMainOutcomes.tex"
...
esttab TenureMHigh_TSJVC TenureMHigh_CSGC TenureMHigh_PWLC TenureMHigh_Exit using "${latex_file}" ///
    ...
    prehead("\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}" "\begin{tabular}{lcccc}" "\toprule \toprule" "& \multicolumn{1}{c}{(1)} & \multicolumn{1}{c}{(2)} & \multicolumn{1}{c}{(3)} & \multicolumn{1}{c}{(4)} \\" "& \multicolumn{1}{c}{Lateral moves} & \multicolumn{1}{c}{Salary grade increases} & \multicolumn{1}{c}{Work-level promotions} & \multicolumn{1}{c}{Exit within 2 years} \\" "\midrule") ///
    posthead("\addlinespace[7pt] & \multicolumn{4}{c}{\textit{Panel (a): worker and manager characteristics}} \\ \addlinespace[7pt] ")
...
esttab OfficeSizeHigh_TSJVC OfficeSizeHigh_CSGC OfficeSizeHigh_PWLC OfficeSizeHigh_Exit using "${latex_file}" ///
    ...
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("\midrule" "\addlinespace[7pt] & \multicolumn{4}{c}{\textit{Panel (b): office and country-wide characteristics}} \\ \addlinespace[7pt] ") posthead("") prefoot("") postfoot("")
...
esttab LowFLFP_TSJVC LowFLFP_CSGC LowFLFP_PWLC LowFLFP_Exit using "${latex_file}" ///
    , append style(tex) fragment nocons nofloat nobaselevels noobs nonumbers ///
    nomtitles collabels(,none) ///
...
esttab WPerf_TSJVC WPerf_CSGC WPerf_PWLC WPerf_Exit using "${latex_file}" ///
    ...
    cells(b(star fmt(3)) se(par fmt(2))) starlevels(* 0.1 ** 0.05 *** 0.01) /// 
    prehead("\midrule" "\addlinespace[7pt] & \multicolumn{4}{c}{\textit{Panel (c): worker performance and moves}} \\ \addlinespace[7pt] ") posthead("") prefoot("") postfoot("")
