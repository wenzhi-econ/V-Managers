...
reghdfe Leaver FT_LtoH FT_HtoH FT_HtoL if selected_event==1, vce(cluster IDlseMHR) absorb(EventTime ISOCodeM)
    eststo Leaver
...
foreach var in $exit_outcomes {
    ...
    reghdfe `var' FT_LtoH FT_HtoH FT_HtoL if EventTime<=${LastPossibleEventTime} & selected_event==1, vce(cluster IDlseMHR) absorb(EventTime ISOCodeM)
        eststo `var'
        test FT_HtoH = FT_HtoL
            local p_Hto = r(p)
            estadd scalar p_Hto = `p_Hto'
...
esttab Leaver $exit_outcomes using "${Results}/PostEventMngrExitOutcomes_Random1.tex", ///
