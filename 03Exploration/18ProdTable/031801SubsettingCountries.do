/* 
This do file explores which countries should be included in the productivity variable.
...
generate one = 1
sort ISOCode IDlse YearMonth
bysort ISOCode: egen Country_Size = total(one) if ProductivityStd!=. & (FT_LtoL==1 | FT_LtoH==1) & LogPayBonus!=.
tab Country_Size

generate Country_Size_500 = (Country_Size >= 500) if Country_Size!=.
generate Country_Size_100 = (Country_Size >= 100) if Country_Size!=.
...
reghdfe ProductivityStd FT_Post FT_LtoH_X_Post if (FT_LtoL==1 | FT_LtoH==1) & LogPayBonus!=., absorb(IDlse YearMonth) cluster(IDlseMHR)
    ...
    ------------+-----------------------------------
            ...
            GTM |        500        0.78        5.95
            HND |        276        0.43        6.38
            IDN |      6,642       10.42       16.79
            IND |     31,704       49.72       66.51
            ITA |      4,670        7.32       73.84
            MEX |      8,305       13.02       86.86
            MYS |        636        1.00       87.86
            NIC |        392        0.61       88.47
            PAN |         39        0.06       88.53
            PHL |      1,042        1.63       90.17
            RUS |      5,250        8.23       98.40
            SGP |          3        0.00       98.41
            SLV |        599        0.94       99.35
            ZAF |        417        0.65      100.00
    ------------+-----------------------------------
...
reghdfe ProductivityStd FT_Post FT_LtoH_X_Post if (FT_LtoL==1 | FT_LtoH==1) & LogPayBonus!=. & Country_Size_500==1, absorb(IDlse YearMonth) cluster(IDlseMHR)
    eststo Prod_500
    summarize ProductivityStd if e(sample)==1 & FT_LtoL==1
    estadd scalar cmean = r(mean)

reghdfe ProductivityStd FT_Post FT_LtoH_X_Post if (FT_LtoL==1 | FT_LtoH==1) & LogPayBonus!=. & Country_Size_100==1, absorb(IDlse YearMonth) cluster(IDlseMHR)
    eststo Prod_100
    summarize ProductivityStd if e(sample)==1 & FT_LtoL==1
    estadd scalar cmean = r(mean)
...
reghdfe ProductivityStd FT_Post FT_LtoH_X_Post if (FT_LtoL==1 | FT_LtoH==1) & Movers_2yrs==1 & LogPayBonus!=. & Country_Size_500==1, absorb(IDlse YearMonth) cluster(IDlseMHR)
    eststo Prod_Movers_500
    summarize ProductivityStd if e(sample)==1 & FT_LtoL==1
    estadd scalar cmean = r(mean)

reghdfe ProductivityStd FT_Post FT_LtoH_X_Post if (FT_LtoL==1 | FT_LtoH==1) & Movers_2yrs==1 & LogPayBonus!=. & Country_Size_100==1, absorb(IDlse YearMonth) cluster(IDlseMHR)
    eststo Prod_Movers_100
    summarize ProductivityStd if e(sample)==1 & FT_LtoL==1
    estadd scalar cmean = r(mean)
...
esttab Prod_500 Prod_Movers_500 LogPayBonus_Movers LogPayBonus_Gain MScore_Gain MScoreB_Gain using "${Results}/ResultsConditionalOnLateralAndVerticalMoves_Size500.tex", ///
    replace style(tex) fragment nocons label nofloat nobaselevels noobs ///
    nomtitles collabels(,none) ///
    star(* 0.10 ** 0.05 *** 0.01) ///
    keep(FT_LtoH_X_Post FT_LtoH) ///
...
esttab Prod_100 Prod_Movers_100 LogPayBonus_Movers LogPayBonus_Gain MScore_Gain MScoreB_Gain using "${Results}/ResultsConditionalOnLateralAndVerticalMoves_Size100.tex", ///
    replace style(tex) fragment nocons label nofloat nobaselevels noobs ///
    nomtitles collabels(,none) ///
    star(* 0.10 ** 0.05 *** 0.01) ///
    keep(FT_LtoH_X_Post FT_LtoH) ///
