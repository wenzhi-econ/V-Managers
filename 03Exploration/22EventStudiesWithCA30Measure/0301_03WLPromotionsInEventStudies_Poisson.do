...
Output:
    "${Results}/005EventStudiesWithCA30/20250502_WLProm_Poisson.txt"
    "${Results}/005EventStudiesWithCA30/CA30_WLPromotions_Poisson.dta"
...
capture log close
log using "${Results}/005EventStudiesWithCA30/20250502_WLProm_Poisson.txt", replace text
...
foreach var in PromWLC {
    ...
    if "`var'" == "PromWLC" global number "6"
    ...
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
    ...
    ppmlhdfe `var' ${four_events_dummies} if Event_Post==1, absorb(IDlse YearMonth) vce(cluster IDlseMHR)
    ...
    *!! quarterly estimates
    ...
    twoway ///
        ...
        legend(off)
    graph save "${Results}/005EventStudiesWithCA30/CA30_Outcome${number}_`var'_Coef1_Gains_Poisson.gph", replace
    ...
    *!! quarterly estimates
    ...
    twoway ///
        ...
        xtitle(Quarters since manager change, size(medlarge)) title("${title}", span pos(12)) ///
        legend(off)
    graph save "${Results}/005EventStudiesWithCA30/CA30_Outcome${number}_`var'_Coef2_Loss_Poisson.gph", replace   
    ...
    *!! quarterly estimates
    ...
    graph save "${Results}/005EventStudiesWithCA30/CA30_Outcome${number}_`var'_Coef3_GainsMinusLoss_Poisson.gph", replace
...
save "${Results}/005EventStudiesWithCA30/CA30_WLPromotions_Poisson.dta", replace 
