/* 
This do file runs event studies on the cumulative sum of task distances: ONETDistC.
...
Output:
    "${Results}/005EventStudiesWithCA30/20250502log_ONETTaskDistance.txt"
    "${Results}/005EventStudiesWithCA30/CA30_ONETDistC.dta"
...
capture log close
log using "${Results}/005EventStudiesWithCA30/20250502log_ONETTaskDistance.txt", replace text
...
foreach var in ONETDistC {
    ...
    if "`var'" == "ONETDistC" global title "Cumulative distance in task intensity"
    if "`var'" == "ONETDistC" global number "7"
    ...
    *!! quarterly estimates
    ...
    twoway ///
        ...
        ylabel(-0.05(0.01)0.05, grid gstyle(dot) labsize(medsmall)) yscale(range(-0.05 0.05)) ///
...
save "${Results}/005EventStudiesWithCA30/CA30_ONETDistC.dta", replace 
