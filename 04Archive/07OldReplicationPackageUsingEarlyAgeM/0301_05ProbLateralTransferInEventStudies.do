/* 
This do file runs event study regressions on the probability of doing different times of lateral moves.

Notice that this variable is only defined after the event, so there cannot be any pre-event values.

Notes on the event study regressions:
    (1) All four treatment groups are included (though Lto and Hto groups do not have same time window), while never-treated workers are not. 
    (2) The omitted group in the regressions are month 0 for all four treatment groups.
    (3) For LtoL and LtoH groups, the relative time period is [0, +84], while for HtoH and HtoL groups, the relative time period is [0, +60].

Some key results (quarterly aggregated coefficients with their p-values, and other key summary statistics) are stored in the output file. 

Input: 
    "${TempData}/04MainOutcomesInEventStudies.dta" <== created in 0104 do file

Output:
    "${Results}/logfile_20241119_ProbLateralTransfer.txt"
    "${Results}/FT_ProbLateralTransfer.dta"

RA: WWZ 
Time: 2024-11-19
*/

capture log close
log using "${Results}/logfile_20241119_ProbLateralTransfer", replace text

use "${TempData}/04MainOutcomesInEventStudies.dta", clear

/* keep if inrange(_n, 1, 10000)  */
    // used to test the codes
    // commented out when officially producing the results

*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*?? step 1. construct outcome variables 
*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??

*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
*-? s-1-1. JobV_AtLeast1 JobV_AtLeast2 JobV_AtLeast3
*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

*!! ignore pre-event transfers 
replace TransferSJV = 0 if FT_Rel_Time<0 & FT_Rel_Time!=.

*!! re-construct the cumulative count: ignore pre-event transfers 
capture drop TransferSJVC
generate temp = TransferSJV
by IDlse (YearMonth), sort: replace temp = temp[_n] + temp[_n-1] if _n>1 
replace temp = 0 if temp==. & StandardJob!=""
generate TransferSJVC = temp 
drop temp

*!! making at least one transfer 
generate JobV_AtLeast1 = 0 if FT_Rel_Time<=0
replace  JobV_AtLeast1 = (TransferSJVC>=1) if FT_Rel_Time>0 & FT_Rel_Time!=.

*!! making at least two transfer 
generate JobV_AtLeast2 = 0 if FT_Rel_Time<=0
replace  JobV_AtLeast2 = (TransferSJVC>=2) if FT_Rel_Time>0 & FT_Rel_Time!=.

*!! making at least three transfer 
generate JobV_AtLeast3 = 0 if FT_Rel_Time<=0
replace  JobV_AtLeast3 = (TransferSJVC>=3) if FT_Rel_Time>0 & FT_Rel_Time!=.

*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
*-? s-1-2. JobV_NoProm1 JobV_NoProm2 JobV_NoProm3
*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

*!! copy the original variables 
generate JobV_NoProm1 = JobV_AtLeast1
generate JobV_NoProm2 = JobV_AtLeast2
generate JobV_NoProm3 = JobV_AtLeast3

*!! identify workers whose work-level has increases after the event 
sort IDlse YearMonth
bysort IDlse: egen Max_PromWL = max(cond(FT_Rel_Time>=0, PromWL, .))

*!! set values to missing for these ever-promoted workers
replace JobV_NoProm1 = . if Max_PromWL==1
replace JobV_NoProm2 = . if Max_PromWL==1
replace JobV_NoProm3 = . if Max_PromWL==1

*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*?? step 2. construct global macros used in regressions 
*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*&& Month 0 is omitted as the reference group.
*impt: The variable name of the "event * relative period" dummies matter!
*impt: Programs stored in the 02*.do files are specifically designed for the following names.
/* Naming patterns:
For normal "event * relative period" dummies, e.g. FT_LtoL_X_Post1 FT_LtoH_X_Post1 FT_HtoH_X_Post12
For binned dummies, e.g. FT_LtoH_X_Post_After84 FT_HtoH_X_Post_After60
*/

*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
*-? s-2-1. event * period dummies 
*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
summarize FT_Rel_Time, detail // range: [-131, +130]

*!! time window of interest
local Lto_max_post_period = 84
local Hto_max_post_period = 60

*!! FT_LtoL
forvalues time = 0/`Lto_max_post_period' {
    generate byte FT_LtoL_X_Post`time' = FT_LtoL * (FT_Rel_Time == `time')
}
generate byte FT_LtoL_X_Post_After`Lto_max_post_period' = FT_LtoL * (FT_Rel_Time > `Lto_max_post_period')

*!! FT_LtoH
forvalues time = 0/`Lto_max_post_period' {
    generate byte FT_LtoH_X_Post`time' = FT_LtoH * (FT_Rel_Time == `time')
}
generate byte FT_LtoH_X_Post_After`Lto_max_post_period' = FT_LtoH * (FT_Rel_Time > `Lto_max_post_period')

*!! FT_HtoH 
forvalues time = 0/`Hto_max_post_period' {
    generate byte FT_HtoH_X_Post`time' = FT_HtoH * (FT_Rel_Time == `time')
}
generate byte FT_HtoH_X_Post_After`Hto_max_post_period' = FT_HtoH * (FT_Rel_Time > `Hto_max_post_period')

*!! FT_HtoL 
forvalues time = 0/`Hto_max_post_period' {
    generate byte FT_HtoL_X_Post`time' = FT_HtoL * (FT_Rel_Time == `time')
}
generate byte FT_HtoL_X_Post_After`Hto_max_post_period' = FT_HtoL * (FT_Rel_Time > `Hto_max_post_period')

*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
*-? s-2-2. global macros used in regressions 
*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

local Lto_max_post_period = 84
local Hto_max_post_period = 60

macro drop FT_LtoL_X_Post FT_LtoH_X_Post FT_HtoH_X_Post FT_HtoL_X_Post

foreach event in FT_LtoL FT_LtoH {
    forvalues time = 1/`Lto_max_post_period' {
        global `event'_X_Post ${`event'_X_Post} `event'_X_Post`time'
    }
    global `event'_X_Post ${`event'_X_Post} `event'_X_Post_After`Lto_max_post_period'
}
foreach event in FT_HtoH FT_HtoL {
    forvalues time = 1/`Hto_max_post_period' {
        global `event'_X_Post ${`event'_X_Post} `event'_X_Post`time'
    }
    global `event'_X_Post ${`event'_X_Post} `event'_X_Post_After`Hto_max_post_period'
}

global four_events_dummies ${FT_LtoL_X_Post} ${FT_LtoH_X_Post} ${FT_HtoH_X_Post} ${FT_HtoL_X_Post}

display "${four_events_dummies}"

    // FT_LtoL_X_Post1 FT_LtoL_X_Post2 ... FT_LtoL_X_Post84 FT_LtoL_X_Pre_After84 
    // FT_LtoH_X_Post1 FT_LtoH_X_Post2 ... FT_LtoH_X_Post84 FT_LtoH_X_Pre_After84 
    // FT_HtoH_X_Post1 FT_HtoH_X_Post2 ... FT_HtoH_X_Post60 FT_HtoH_X_Pre_After60 
    // FT_HtoL_X_Post1 FT_HtoL_X_Post2 ... FT_HtoL_X_Post60 FT_HtoL_X_Pre_After60 

*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
*-? s-2-3. FT_Post variable 
*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

generate FT_Post = (FT_Rel_Time>=0) if FT_Rel_Time!=.

*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??
*?? step 3. regressions on the probabilities of making exact number of lateral moves
*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??*??

foreach var in JobV_AtLeast1 JobV_AtLeast2 JobV_AtLeast3 JobV_NoProm1 JobV_NoProm2 JobV_NoProm3 {

    if "`var'" == "JobV_AtLeast1" global title "Probability of at least 1 lateral move"
    if "`var'" == "JobV_AtLeast2" global title "Probability of at least 2 lateral moves"
    if "`var'" == "JobV_AtLeast3" global title "Probability of at least 3 lateral moves"

    if "`var'" == "JobV_NoProm1"  global title "Prob. of at least 1 lateral move (cond. on no WL promotions)"
    if "`var'" == "JobV_NoProm2"  global title "Prob. of at least 2 lateral moves (cond. on no WL promotions)"
    if "`var'" == "JobV_NoProm3"  global title "Prob. of at least 3 lateral moves (cond. on no WL promotions)"

    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
    *-? step 1. Main Regression
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

    reghdfe `var' ${four_events_dummies} if (FT_Mngr_both_WL2==1 & FT_Post==1), absorb(IDlse YearMonth) vce(cluster IDlseMHR) 
    
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
    *-? step 2. LtoH versus LtoL
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

    *!! quarterly estimates
    LH_minus_LL_OnlyPost, event_prefix(FT) post_window_len(84) outcome(`var')
    twoway ///
        (scatter coeff_`var'_gains quarter_`var'_gains, lcolor(ebblue) mcolor(ebblue)) ///
        (rcap lb_`var'_gains ub_`var'_gains quarter_`var'_gains, lcolor(ebblue)) ///
        , yline(0, lcolor(maroon)) xline(0, lcolor(maroon)) ///
        xlabel(0(2)28, grid gstyle(dot)) ///
        ylabel(, grid gstyle(dot)) ///
        xtitle(Quarters since manager change) title("${title}", span pos(12)) ///
        legend(off)
    graph save "${Results}/FT_Gains_AllEstimates_`var'.gph", replace
    
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
    *-? step 3. HtoL versus HtoH
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

    *!! quarterly estimates
    HL_minus_HH_OnlyPost, event_prefix(FT) post_window_len(60) outcome(`var')
    twoway ///
        (scatter coeff_`var'_loss quarter_`var'_loss, lcolor(ebblue) mcolor(ebblue)) ///
        (rcap lb_`var'_loss ub_`var'_loss quarter_`var'_loss, lcolor(ebblue)) ///
        , yline(0, lcolor(maroon)) xline(0, lcolor(maroon)) ///
        xlabel(0(2)20, grid gstyle(dot)) ///
        ylabel(, grid gstyle(dot)) ///
        xtitle(Quarters since manager change) title("${title}", span pos(12)) ///
        legend(off)
    graph save "${Results}/FT_Loss_AllEstimates_`var'.gph", replace   

    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
    *-? step 4. Testing for asymmetries
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?

    *!! post-event joint p-value
    postevent_Double_Diff_OnlyPost, event_prefix(FT) post_window_len(60)
        global postevent_`var' = r(postevent)
        global postevent_`var' = string(${postevent_`var'}, "%4.3f")
        generate postevent_`var' = ${postevent_`var'} if inrange(_n, 1, 41)

    *!! quarterly estimates
    Double_Diff_OnlyPost, event_prefix(FT) post_window_len(60) outcome(`var')
    twoway ///
        (scatter coeff_`var'_ddiff quarter_`var'_ddiff, lcolor(ebblue) mcolor(ebblue)) ///
        (rcap lb_`var'_ddiff ub_`var'_ddiff quarter_`var'_ddiff, lcolor(ebblue)) ///
        , yline(0, lcolor(maroon)) xline(0, lcolor(maroon)) ///
        xlabel(0(2)20, grid gstyle(dot)) ///
        ylabel(, grid gstyle(dot)) ///
        xtitle(Quarters since manager change) title("${title}", span pos(12)) ///
        legend(off) note("Post coeffs. joint p-value = ${postevent_`var'}")
    graph save "${Results}/FT_GainsMinusLoss_AllEstimates_`var'.gph", replace
}

keep ///
    coeff_* quarter_* lb_* ub_* postevent_* ///
    LtoL_* LtoH_* HtoH_* HtoL_* ///
    coef1_* coefp1_* coef2_* coefp2_* coef3_* coefp3_* coef4_* coefp4_* coef5_* coefp5_* coef6_* coefp6_* ///
    RI1_* rip1_* RI2_* rip2_* RI3_* rip3_*

keep if inrange(_n, 1, 41)

save "${Results}/FT_ProbLateralTransfer.dta", replace 


log close 