...
foreach v in LogPayBonus PromWLC ChangeSalaryGradeC TransferSJC TransferSJVC TransferSJLLC{
	
	reghdfe `v', a(IDlse YearMonth) res(`v'R)
}

xtset IDlse YearMonth 
foreach v in  TransferSJC TransferSJVC TransferSJLLC TransferSJCR TransferSJVCR TransferSJLLCR{
gen l`v' = l1.`v'	
}
...
gen bb = t in 4
gen bbb= d in 4
tw rspike lo hi  ttt, legend(off) xlabel() lcolor(ebblue) lwidth(2 ..) horizontal || bar bb ttt , horizontal fcolor(white) lcolor(ebblue) || bar bbb ttt , bcolor(ebblue) horizontal   || dot coeff ttt,  horizontal  ndots(0)  msymbol(d) mcolor(white)  xtitle("Impact of good manager on long term career progression") xtick(0) ylabel(none) ytitle("") ylabel(0 "Percent decomposition" 1 "Total effect" 2 "Direct effect" 3 "Indirect effect", add custom  ) dotextend(no) aspect(0.3) text(0.1 0.04 "40%") graphregion(margin(0 0 0 0))
...
twoway rarea _med_updelta0 _med_lodelta0 _med_rho, bcolor(ltblue) || line _med_delta0 _med_rho, lcolor(ebblue) ytitle("Average Causal Mediation Effect (ACME)") title("Average Causal Mediation Effect (ACME) and {&rho}") xtitle("Sensitivity parameter: {&rho}") legend(off)  yline(0, lpattern(solid)) xline(0, lpattern(solid)) xlabel(-1(0.2)1)  ylabel(-0.3(0.1)0.3) ///
 xline( `rho', lcolor(maroon)) xlabel(`rho' "{bf: {&rho} = `rho'}", add custom labcolor(maroon) angle(45)) yline(`ACME', lcolor(ebblue))  ylabel(`ACME' "{bf:ACME=`ACME'}", add custom labcolor(ebblue) angle(45)) 
