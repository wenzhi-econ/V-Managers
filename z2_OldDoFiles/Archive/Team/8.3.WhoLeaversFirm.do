********************************************************************************
* Profiles of people who exit firm 
********************************************************************************
...
bys IDlse: egen ExitEvent = mean(cond(LeaverPerm==1, YearMonth,. )) // exit month 
format  ExitEvent %tm 
...
balancetable   (mean if PromSG75LHPost==1) (mean if PromSG75LLPost==1) (diff LHPost if LHPost!=. ) (mean if PromSG75HLPost==1) (mean if PromSG75HHPost==1) (diff HLPost if HLPost!=. )  $charsCoef   if Diff<=36 using "$analysis/Results/8.Team/LeaverPromSG75.tex", ///
...
global most    Female Age20  LogTenure  LogPayBonus  ChangeSalaryGradeC EarlyAge NewHire LeaverVol LeaverInv
...
coefplot $most , keep(HLPost) ci(90)  xline(0 ,lpattern(solid) lcolor(black))  scheme(white_tableau)  aseq swapnames legend(off) note("Notes. Standard errors clustered at the manager level." "Controlling for country, year and function FE." "Exit under the  manager of the transition event.", size(small))   headings(Female = "{bf:Demographics}" LogTenure = "{bf:Performance at work}" , labgap(0)) title("Leavers (firm): High to Low versus High to High")
graph export "$analysis/Results/8.Team/ExitHL.png", replace 
graph save "$analysis/Results/8.Team/ExitHL.gph", replace 
