********************************************************************************
* Profiles of people who exit either team or firm 
********************************************************************************
...
bys IDlse: egen ExitEvent = mean(cond(LeaverPerm==1 | F1ChangeM==1, YearMonth,. )) // exit or exit team  month 
format  ExitEvent %tm 
...
balancetable   (mean if PromSG75LHPost==1) (mean if PromSG75LLPost==1) (diff LHPost if LHPost!=. ) (mean if PromSG75HLPost==1) (mean if PromSG75HHPost==1) (diff HLPost if HLPost!=. )  $charsCoef   if Diff<=36 using "$analysis/Results/8.Team/LeaverTeamPromSG75.tex", ///
...
* FIGURES 
*global charsCoef Female  Age20      LogTenure LogPayBonus ChangeSalaryGradeC EarlyAge NewHire LeaverVol LeaverInv
...
* for education, taking out func FE as there is no variation in education within func 
foreach var in  $edu {
	eststo `var': reghdfe `var' LHPost if  Diff>2   & SameMEvent==1 , a( $FE  i.Year  ) vce(cluster IDlseMHR)  // using 12 is too few exits 
}

coefplot $most , keep(LHPost) ci(90)  xline(0 ,lpattern(solid) lcolor(black))  scheme(white_tableau)  aseq swapnames legend(off) note("Notes. Standard errors clustered at the manager level." "Controlling for country, year and function FE." "Exit under the manager of the transition event (exit team or firm).", size(small))   headings(Female = "{bf:Demographics}" LogTenure = "{bf:Performance at work}" , labgap(0)) title("Leavers, changing team or exiting firm:" "Low to High versus Low to Low")
graph export "$analysis/Results/8.Team/LeaverTeamLH.png", replace 
graph save "$analysis/Results/8.Team/LeaverTeamLH.gph", replace 

eststo clear 
foreach var in  $most {
	eststo `var': reghdfe `var' HLPost if  Diff>2   & SameMEvent==1 , a( $FE  $cont   ) vce(cluster IDlseMHR)  // using 12 is too few exits 
}

coefplot $most , keep(HLPost) ci(90)  xline(0 ,lpattern(solid) lcolor(black))  scheme(white_tableau)  aseq swapnames legend(off) note("Notes. Standard errors clustered at the manager level." "Controlling for country, year and function FE." "Exit under the  manager of the transition event (exit team or firm).", size(small))   headings(Female = "{bf:Demographics}" LogTenure = "{bf:Performance at work}" , labgap(0)) title("Leavers, changing team or exiting firm:"  "High to Low versus High to High")
graph export "$analysis/Results/8.Team/LeaverTeamHL.png", replace 
graph save "$analysis/Results/8.Team/LeaverTeamHL.gph", replace 
