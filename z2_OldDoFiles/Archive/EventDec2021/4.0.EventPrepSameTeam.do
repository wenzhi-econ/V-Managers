...
* manager change 
gen  ChangeMR = 0 
replace ChangeMR = 1 if ChangeM==1 & MaxWLM>1 
...
replace ChangeMR = 0 if tt!=TeamSize // 30K events 

drop Ei tt // recreate Ei using update manager transition definition 
gen    Ei = YearMonth if ChangeMR==1 // time of event 
format Ei %tm 
...
ta ChangeMR PropSameTeamAll, row // 84% of the changes are instances where manager inherits whole team! 
