...
5. Same as 1, but for UniVoice

6. Same as 2, but for UniVoice

7. Same as 4, but for UniVoice

Input: UniVoiceSnapshotWC.dta
Output: UniVoiceSnapshotWCMIDlse.dta
...
save "AllSnapshotWCM.dta",replace
...
if "`c(filename)'" != "$dta/AllSnapshotWCM.dta" | `c(changed)' == 1 {
...
*********************************************************************************
* 5. UniVoice: Identifying managers in the Original Dataset
*********************************************************************************

* 5.a. I identify managers in UniVoiceSnapshotWC.dta using the tempfile.

use "UniVoiceSnapshotWC.dta", clear
merge 1:1 IDlse YearMonth using "$Managersdta/Temp/Mlist.dta"
...
* 5.c. saving as UniVoiceSnapshotWCM.dta
save "UniVoiceSnapshotWCM.dta",replace

*********************************************************************************
* 6. UniVoice: Adding ISOCodeHome.
*********************************************************************************

* use UniVoiceSnapshotWCM.dta if it is not loaded or loaded but changed
if "`c(filename)'" != "$dta/UniVoiceSnapshotWCM.dta" | `c(changed)' == 1 {
use "$dta/UniVoiceSnapshotWCM.dta",clear
}

* 6.a. Merging isocode.dta with the original data.
...
* 6.b. Updating UniVoiceSnapshotWCMIDlse.dta
save "UniVoiceSnapshotWCM.dta",replace

*********************************************************************************
* 7. UniVoice: Merging UniVoiceSnapshotWCIDlseM with Mchar and adding new variables
*********************************************************************************

* use UniVoiceSnapshotWCMIDlse.dta if it is not loaded or loaded but changed
if "`c(filename)'" != "$dta/UniVoiceSnapshotWCM.dta" | `c(changed)' == 1 {
use "$dta/UniVoiceSnapshotWCM.dta",clear
}
* 7.a. Adding MListchar variables by merging the tempfile with UniVoiceSnapshotWCM.dta
...
* Office
distinct Office // number of establishments 2550 (for WC); 902 (for HRUniVoice)
distinct Country // number of countries: 117 (for WC); 113 (for HRUniVoice)
quietly bys Office: gen dup_location = cond(_N==1,0,_n)
bys Country YearMonth: egen OfficeNum = count(Office) if (dup_location ==0 & Office !=. | dup_location ==1 & Office !=.) 
label var OfficeNum "No. of offices in each Country and Month"
...
* 7.c. Dropping the duplicates, compressing and saving in UniVoiceSnapshotWCM.dta.

duplicates drop IDlse YearMonth, force // the duplicates have . for IDlse. I drop these
