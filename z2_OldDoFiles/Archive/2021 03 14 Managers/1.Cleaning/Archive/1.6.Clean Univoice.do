...
Input: UniVoiceSnapshot.dta
Output: UniVoiceSnapshotM.dta
*/
...
* 5.a. I identify managers in UniVoiceSnapshot.dta using the tempfile.

use "UniVoiceSnapshot.dta", clear
...
* 5.c. saving as UniVoiceSnapshotM.dta
save "UniVoiceSnapshotM.dta",replace
...
if "`c(filename)'" != "$dta/UniVoiceSnapshotM.dta" | `c(changed)' == 1 {
use "$dta/UniVoiceSnapshotM.dta",clear
}
...
* 7.c. Dropping the duplicates, compressing and saving in UniVoiceSnapshotM.dta.

duplicates drop IDlse YearMonth, force // the duplicates have . for IDlse. I drop these

compress

save "$data/dta/UniVoiceSnapshotM.dta",replace
