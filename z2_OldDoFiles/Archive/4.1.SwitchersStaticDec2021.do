...
* PromWLVC
eststo clear 
foreach v in LogPayBonus ChangeSalaryGradeC PromWLC VPA VPA125  {
...
count if  `v' !=. & IDlseMHR!=. 
local N1 = r(N)
eststo: reghdfe `v' ELLPost ELHPost EHLPost EHHPost c.Tenure##c.Tenure c.TenureM##c.TenureM TeamSize , a( AgeBand Year AgeBandM  IDlse) cluster(IDlseMHR)
...
esttab using "$analysis/Results/4.Switchers/Perf.tex",  label star(* 0.10 ** 0.05 *** 0.01) se r2 ///
s(cmean N1 r2 pvalue1 pvalue2, labels( "Mean" "\hline N" "R-squared" "\hline E\textsubscript{LL} = E\textsubscript{LH}" "E\textsubscript{HL} = E\textsubscript{HH}" ) ) interaction("$\times$ ")  nobaselevels  drop(_cons TeamSize Tenure c.Tenure#c.Tenure TenureM c.TenureM#c.TenureM ) ///
 nomtitles mgroups("Pay+Bonus (logs)" "Prom. (salary)" "Prom. (work-level)" "Perf. Appraisal" "Top Perf. Appraisal" "Exit" , pattern(1 1 1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span}))  ///
...
eststo clear 
foreach v in  LeaverPerm LeaverVol LeaverInv  {
local lbl : variable label `v'
mean `v' if IDlseMHR!=. 
mat coef=e(b)
local cmean = coef[1,1]
count if  `v' !=. & IDlseMHR!=. 
local N1 = r(N)
eststo: reghdfe `v' ELLPost ELHPost EHLPost EHHPost c.Tenure##c.Tenure c.TenureM##c.TenureM TeamSize , a( AgeBand CountryYM AgeBandM ) cluster(IDlseMHR)
test  ELHPost = ELLPost
estadd scalar pvalue1 = r(p)
test  EHLPost = EHHPost
estadd scalar pvalue2 = r(p)
...
* more transfers but entirely under different manager (under same manager there is no difference)
esttab using "$analysis/Results/4.Switchers/Exit.tex",  label star(* 0.10 ** 0.05 *** 0.01) se r2 ///
s(cmean N1 r2 pvalue1 pvalue2, labels( "Mean" "\hline N" "R-squared" "\hline E\textsubscript{LL} = E\textsubscript{LH}" "E\textsubscript{HL} = E\textsubscript{HH}" ) ) interaction("$\times$ ")  nobaselevels  drop(_cons TeamSize Tenure c.Tenure#c.Tenure TenureM c.TenureM#c.TenureM ) ///
 nomtitles mgroups("Exit" "Exit (Vol.)" "Exit (Inv.)" , pattern(1 1 1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span}))  ///
nofloat nonotes postfoot("\hline" "\end{tabular}" "\begin{tablenotes}" "\footnotesize" "\item" ///
Notes. An observation is a employee-month. Controls include: employee FE, country x time FE, manager FE, team size, and age group FE, tenure and tenure squared of manager and employee. ///
"\end{tablenotes}") replace
...
label var EHHPost "Post E\textsubscript{HH}"
eststo clear 
foreach v in TransferInternalLLC  TransferInternalVC TransferInternalSameMC  TransferInternalDiffMC TransferFuncC ONETActivitiesDistanceCB1 {
local lbl : variable label `v'
mean `v' if IDlseMHR!=. 
mat coef=e(b)
local cmean = coef[1,1]
count if  `v' !=. & IDlseMHR!=. 
local N1 = r(N)
eststo: reghdfe `v' ELLPost ELHPost EHLPost EHHPost c.Tenure##c.Tenure c.TenureM##c.TenureM TeamSize , a( AgeBand CountryYM AgeBandM  IDlse) cluster(IDlseMHR)
test  ELHPost = ELLPost
estadd scalar pvalue1 = r(p)
test  EHLPost = EHHPost
estadd scalar pvalue2 = r(p)
...
* more transfers but entirely under different manager (under same manager there is no difference)
esttab using "$analysis/Results/4.Switchers/Transfers.tex",  label star(* 0.10 ** 0.05 *** 0.01) se r2 ///
s(cmean N1 r2 pvalue1 pvalue2, labels( "Mean" "\hline N" "R-squared" "\hline E\textsubscript{LL} = E\textsubscript{LH}" "E\textsubscript{HL} = E\textsubscript{HH}" ) ) interaction("$\times$ ")  nobaselevels  drop(_cons TeamSize Tenure c.Tenure#c.Tenure TenureM c.TenureM#c.TenureM ) ///
 nomtitles mgroups("Transfers (lateral)" "Transfers (vertical)" "Same M." "Diff. M."  "Transfer Function" "Task distance > 0" , pattern(1 1 1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span}))  ///
nofloat nonotes postfoot("\hline" "\end{tabular}" "\begin{tablenotes}" "\footnotesize" "\item" ///
Notes. An observation is a employee-month. Controls include: employee FE, country x time FE, manager FE, team size, and age group FE, tenure and tenure squared of manager and employee. ///
"\end{tablenotes}") replace
