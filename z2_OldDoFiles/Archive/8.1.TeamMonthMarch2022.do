...
foreach y in  $perf $move  $homo $div $job   {
...
keep(*DEvent) vertical recast(bar )  scheme(burd5) citop legend(off) ///
...
coefplot  reg6FE  reg7FE  , levels(90) ///
keep(*DEvent) vertical recast(bar ) rescale(100)  ciopts(recast(rcap)) citop legend(off) ///
msymbol(d) mcolor(white) swapnames aseq ///
coeflabels(  reg5FE = "{bf:Exit Firm}"  reg6FE = "{bf:Change Team}" reg7FE = "{bf:Join Team}" reg8FE = "{bf:Job change, same team}" ) ///
note("Notes. An observation is a team-month. Controls include: team fixed effects, tenure and tenure squared of" "manager interacted with gender. Standard errors clustered at the manager level." "Reporting 90% confidence intervals.", span) ///
title("" " ", pos(12) span si(vlarge)) ytitle(Percentage points) yscale(range(0 1)) ylabel(0(0.2)1)
*ysize(6) xsize(8)  ytick(#6,grid glcolor(black)) scale(0.9) yscale(range(0 1.2)) ylabel(0(0.2)1.2) ytitle(Percentage points)
graph export  "$analysis/Results/8.Team/`Label'MoveSym1a.png", replace

coefplot reg5FE reg8FE , levels(90) ///
keep(*DEvent) vertical recast(bar ) rescale(100)  ciopts(recast(rcap)) citop legend(off) ///
msymbol(d) mcolor(white) swapnames aseq ///
coeflabels(  reg5FE = "{bf:Exit Firm}"  reg6FE = "{bf:Change Team}" reg7FE = "{bf:Join Team}" reg8FE = "{bf:Job change, same team}" ) ///
note("Notes. An observation is a team-month. Controls include: team fixed effects, tenure and tenure squared of" "manager interacted with gender. Standard errors clustered at the manager level." "Reporting 90% confidence intervals.", span) ///
title("" " ", pos(12) span si(vlarge)) ytitle(Percentage points)
*ysize(6) xsize(8)  ytick(#6,grid glcolor(black)) scale(0.9) yscale(range(0 1.2)) ylabel(0(0.2)1.2) ytitle(Percentage points)
graph export  "$analysis/Results/8.Team/`Label'MoveSym1b.png", replace
