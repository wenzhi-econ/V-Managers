...
gen sampleN = (LineManagerB!=.)&  (Uteampc1!=.)& (Ufocuspc1!=.)& (Uhappypc1!=.)& (Ucompanypc1!=.)

eststo clear 
foreach var in LineManagerB Uteampc1 Ufocuspc1 Uhappypc1 Ucompanypc1 { // Whealthpc1 Wtalkpc1 Wjobpc1 Wawarepc1  Wusepc1
eststo `var':reghdfe `var'  EarlyAgeM  if Post==1 & WLM==2 & sampleN==1, cluster(IDlseMHR) a(  AgeBand##Female Year Country  )
qui sum `e(depvar)' if (e(sample) & Post==1 & EarlyAgeM==0 & WLM==2)
...
Notes. An observation is a worker-year-month. Data from the annual pulse survey run by the firm since 2017. Standard errors are clustered by manager. Controls include: age-groupxgender FE, and country and year FE. Survey indices are the first principal components of various survey questions, grouped together by theme as detailed in Table \ref{tab:construction}. I use binary variables: probability of answering 5 out of 5-point Likert Scale. Estimates obtained by running the model in equation \ref{eq:static}. Table \ref{tab:SurveyMean} Panel (a) shows that the results are very similar when using simple averages for the indices instead of the first principal component.  ///
"\end{tablenotes}")
...
eststo `var':reghdfe `var'  EarlyAgeM  if Post==1 & WLM==2 & sampleN==1 , cluster(IDlseMHR) a(  AgeBand##Female Year Country  )
qui sum `e(depvar)' if e(sample) &  EarlyAgeM  ==0 & Post==1 & WLM==2 
estadd scalar Mean = r(mean)	
}
* outcome mean low flyer
...
**# ON PAPER TABLE: SurveyMean.tex (Top Panel A)
...
posthead("\hline \\ \multicolumn{6}{c}{\textit{Panel (a): using averages for the indices}} \\\\[-1ex]") ///
fragment ///
cells(b(star fmt(4)) se(par fmt(4))) starlevels(* 0.1 ** 0.05 *** 0.01) ///
stats(Mean r2, fmt(3 4) labels("Mean, low-flyer"  "R-squared")) ///
label nofloat nonotes collabels(none) drop(_cons) ///
mtitles("Effective Leader" "Team Effectiveness" "Job Satisfaction" "Autonomy" "Company Effectiveness") 
...
eststo clear 
foreach var in LineManagerB Uteampc1 Ufocuspc1 Uhappypc1 Ucompanypc1 { // Whealthpc1 Wtalkpc1 Wjobpc1 Wawarepc1  Wusepc1 
eststo `var':reghdfe `var'  EarlyAgeM  if Post==1 & WLM==2 & KEi<=12 , cluster(IDlseMHR) a(  AgeBand##Female Year Country  )	
}
...
eststo `var':reghdfe `var'  EarlyAgeM  if Post==1 & KEi <=12  & WLM==2 , cluster(IDlseMHR) a(  AgeBand##Female Year Country  )	
}
...
reghdfe DevOpportunityB  EarlyAgeM  if Post==1 & WLM==2 , cluster(IDlseMHR) a(  AgeBand##Female Year Country  )
...
eststo clear 
foreach var in   LineManagerB  Uteampc1 Ufocuspc1 Uhappypc1 Ucompanypc1  { // Whealthpc1 Wtalkpc1 Wjobpc1 Wawarepc1  Wusepc1
eststo `var':reghdfe `var'  EarlyAgeM##maxTransfer  if Post==1 & WLM==2 & sampleN==1 , cluster(IDlseMHR) a(  AgeBand##Female Year Country  )
...
label var EarlyAgeM "High-flyer"
**# ON PAPER TABLE: SurveyMean.tex (Bottom Panel B)
estadd local label 1.EarlyAgeM#1.maxTransfer "High-flyer × Worker changed job"

esttab LineManagerB Uteampc1 Uhappypc1 Ufocuspc1 Ucompanypc1 using "$analysis/Results/0.Paper/3.2.Survey/SurveyMean.tex", ///
posthead("\hline \\ \multicolumn{6}{c}{\textit{Panel (b): heterogeneous effects by whether worker changes job}} \\\\[-1ex]") ///
fragment ///
cells(b(star fmt(4)) se(par fmt(4))) starlevels(* 0.1 ** 0.05 *** 0.01) ///
stats(Mean r2 N, fmt(3 4 0) labels("Mean, low-flyer" "R-squared" "N")) ///
...
append ///
postfoot("\hline" "\end{tabular}" "\begin{tablenotes}" "\footnotesize" "\item" ///
Notes. An observation is a worker-year-month. Data from the annual pulse survey run by the firm since 2017. Standard errors are clustered by manager. Controls include: age-groupxgender FE, and country and year FE. In Panel (a), survey indices are the average of various survey questions, grouped together by theme as detailed in Table \ref{tab:construction}. I use binary variables: probability of answering 5 out of 5-point Likert Scale.  Estimates obtained by running the model in equation \ref{eq:static}. In Panel (b), survey indices are the first principal components of various survey questions, grouped together by theme as detailed in Table \ref{tab:construction} and sample is restricted to the first year since the manager transition. Estimates obtained by running the model in equation \ref{eq:static} interacting indicator for high-flyer manager with an indicator for whether the worker changes job. ///
"\end{tablenotes}") 
...
eststo `var':reghdfe `var'  EarlyAgeM##maxTransfer  if Post==1 & WLM==2 , cluster(IDlseMHR) a(  AgeBand##Female Year Country  )
*eststo `var':reghdfe `var'  EarlyAgeM##maxTransferLL  if Post==1 & WL2==1, cluster(IDlseMHR) a( $control Year Country )	
}
