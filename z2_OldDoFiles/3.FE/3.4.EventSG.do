...
gen diffM = d.MFEBayes // can be replace with d.EarlyAge2015M 
...
* TEAM SIZE AND PERFORMANCE // code to be tried out 
xtset IDlse YearMonth 
gen PayGrowth = d.LogPayBonus
bys IDlseMHR YearMonth: egen TeamPayG = mean(PayGrowth)
egen tt = tag(IDlseMHR YearMonth)
su TeamPayG if tt==1 , d 
gen AboveTeamPayG = TeamPayG > r(p50) if TeamPayG !=.
su TeamSize if tt==1, d 
gen AboveTeamSize =TeamSize > r(p50) if TeamSize !=.
drop tt 

gen c1 = AboveTeamSize if YearMonth == Ei
bys IDlse: egen AboveTeamSize0 = mean(c1)
gen c2 = AboveTeamPayG if YearMonth == Ei
bys IDlse: egen AboveTeamPayG0 = mean(c2)
drop c1 c2 
...
graph save  "$analysis/Results/4.Event/SG/TWFE`var'.gph", replace
graph export "$analysis/Results/4.Event/SG/TWFE`var'.png", replace 
}
...
graph save  "$analysis/Results/4.Event/SG/TWFE`var'LL.gph", replace
graph export "$analysis/Results/4.Event/SG/TWFE`var'LL.png", replace 
}
...
*note("Pretrends p-value=`jointDeltaM'") ylabel(0 "0(`ymeanF1')", add custom labcolor(maroon))
graph save  "$analysis/Results/4.Event/SG/TWFE`var'TeamG.gph", replace
graph export "$analysis/Results/4.Event/SG/TWFE`var'TeamG.png", replace 
}
...
graph save  "$analysis/Results/4.Event/SG/TWFEProdDeltaM.gph", replace
graph export "$analysis/Results/4.Event/SG/TWFEProdDeltaM.png", replace 
...
graph save  "$analysis/Results/4.Event/SG/Imputation/DIDProdDeltaM.gph", replace
graph export  "$analysis/Results/4.Event/SG/Imputation/DIDProdDeltaM.png", replace
