...
egen Ind_tag  = tag(IDlse)
    //&? Neither of these two variables matter.
    //&? (see the TenureMHigh1 example on why the two tags are not used even if we are caring about managers' characteristics)
...
* impt: The calculation is calculated using full panel data, so one worker (and thus one manager) can appear multiple times in calculation.
...
summarize FMShareWB1, detail
generate LowWholeFLFP1 = 1 if FMShareWB1<=r(p50)
replace  LowWholeFLFP1 = 0 if LowWholeFLFP1==. & FMShareWB1!=.
