...
Output:
    "${Results}/logfile_20241123_EndogenousMobilityChecks_ToH.txt"
...
label variable FT_toH "High-flyer manager"
...
foreach y in  $perf $div $homo {
    ...
    reghdfe `y' FT_toH if spanM>1 & inrange(FT_Rel_Time, -36, -6) & FT_Mngr_both_WL2==1, cluster(IDlseMHRPreMost) absorb($controls)
        eststo reg`i'
        summarize `y' if e(sample)==1 & FT_toL==1
...
esttab reg1 reg2 reg3 reg4 using "${Results}/EndogenousMobilityChecks_ToH_WorkerAndTeamRestrictions.tex", ///
...
esttab reg5 reg6 reg7 reg8 using "${Results}/EndogenousMobilityChecks_ToH_WorkerAndTeamRestrictions.tex", ///
...
esttab reg9 reg10 reg11 reg12 using "${Results}/EndogenousMobilityChecks_ToH_WorkerAndTeamRestrictions.tex", ///
    append fragment nofloat nonotes nocons label nobaselevels interaction("$\times$") ///
    keep(FT_toH) ///
    star(* 0.10 ** 0.05 *** 0.01) b(4) se(3) ///
    stats(mean_toL r2 N, labels( "Mean, low-flyer manager" "N" "R-squared") fmt(%9.3f %9.3f %9.0g)) ///
