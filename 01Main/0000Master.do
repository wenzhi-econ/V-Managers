...
It includes the following code blocks:
    default setups specifications (e.g., version specification)
    paths specifications
    installation of necessary packages, and usage of local libraries
    figure scheme specifications
    do files used to replicate output in the paper (the dofile-output mapping is in the _Description.md file) 
...
do "${DoFiles}/01Main/0207CoefPrograms_CohortDynamics.do" */
    //&? Above do files need to be executed before the following two code blocks: s-5-3-2, and s-5-3-3.
