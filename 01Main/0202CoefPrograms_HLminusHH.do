...
summarize `outcome' if e(sample)==1 & inrange(`event_prefix'_Rel_Time, -3, -1) & (`event_prefix'_HtoH==1)
    local HtoH_base_mean = r(mean)
    generate HtoH_`outcome' = `HtoH_base_mean' if inrange(_n, 1, `total_quarters')

summarize `outcome' if e(sample)==1 & inrange(`event_prefix'_Rel_Time, -3, -1) & (`event_prefix'_HtoL==1)
