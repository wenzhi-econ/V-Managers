...
capture log close
log using "${Results}/logfile_20241226_TwoMainOutcomes_CohortDynamics_HtoLvsHtoH", replace text
...
foreach var in TransferSJVC ChangeSalaryGradeC {
    ...
    *-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?*-?
    ...
    reghdfe `var' ${eventinteract} if (FT_Mngr_both_WL2==1 & FT_Never_ChangeM==0) & (FT_HtoL==1 | FT_HtoH==1), absorb(IDlse YearMonth) vce(cluster IDlseMHR) 
    ...
    *!! quarterly estimates
    ...
    twoway ///
        ...
        legend(off)
    graph save "${Results}/FT_Loss_AllEstimates_`var'_CohortDynamics_HtoLvsHtoH.gph", replace   
...
save "${Results}/FT_TwoMainOutcomes_CohortDynamics_HtoLvsHtoH.dta", replace 
